...
class Editor
{
	...
	create_file_button( filename )
	{
		...
		close_button.title     = "close file"
		close_button.alt       = "close file"
	...
	create_window( filename, text )
	{
		...
		if ( filename_to_kind( filename ) === "HTML" ) { new_session.setUseWorker( false ); }
		// Set the editor to the correct language.
		if ( filename.endsWith( ".js" ) ) { new_session.setMode( "ace/mode/javascript" ); }
		else if ( filename.endsWith( ".css" ) ) { new_session.setMode( "ace/mode/css" ); }
		else if ( filename.endsWith( ".html" ) ) { new_session.setMode( "ace/mode/html" ); }
...
class Navigation
{
	constructor()
	{
		...
		this.new_menu_items = [
			this.make_button,
			this.filename_textfield,
			this.back_div,
			this.filename_input_label,
			this.info_text
		];
